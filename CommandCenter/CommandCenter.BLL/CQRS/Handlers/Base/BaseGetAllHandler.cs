...
namespace CommandCenter.BLL.CQRS.Handlers.Base
{
        ...
        where TModelGet : class, IBaseResource
    {
        ...
        {
            _logger.LogInformation($"Trying to fetch Collection of {typeof(TEntity)} from a datastore ...");

            var entities = await _repository.GetAllAsync();
            var models = _dataMapper.MapCollection<TEntity, TModelGet>(entities);

            _logger.LogInformation($"Collection of {typeof(TModelGet)} were received successfully.");
